  ...
  try {
    ...
    if (method === 'PATCH') {
      ...
      if (action === 'change_password') {
        ...
        try {
          const isPasswordValid = compareSync(currentPassword, admin.password_hash);
          ...
          if (updateError) {
            return new Response(
              JSON.stringify({ error: 'Error updating password', details: updateError.message }),
              {
                status: 500,
                headers: { ...corsHeaders, 'Content-Type': 'application/json' },
              }
          ...
          return new Response(
            JSON.stringify({ error: 'Error processing password', details: String(bcryptError) }),
            {
              status: 500,
              headers: { ...corsHeaders, 'Content-Type': 'application/json' },
            }
    ...
    if (method === 'DELETE') {
      const pathParts = url.pathname.split('/');      const userId = pathParts[pathParts.length - 1];
