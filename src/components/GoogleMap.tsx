...
export function GoogleMap({ lat, lng, address }: GoogleMapProps) {
  ...
  if (!isConfigured) {
    return (
      <div className="bg-dark-200 rounded-xl border border-white/10 p-6">
  ...
  return (
    <div className="bg-dark-200 rounded-xl border border-white/10 overflow-hidden">
      ...
      <div
        ref={mapRef}
        className="w-full h-64 bg-dark-100"
      />

      <div className="p-4 flex gap-2">
        <button
          onClick={handleOpenInMaps}
          className="flex-1 px-4 py-2 bg-dark-100 hover:bg-dark-50 text-white rounded-lg transition-colors flex items-center justify-center gap-2 border border-white/10"
        >
          <MapPin className="w-4 h-4" />
          Open in Maps
        </button>
