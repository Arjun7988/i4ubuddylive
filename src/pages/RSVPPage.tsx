...
import { Plus, Calendar, MapPin, Users, CheckCircle, XCircle, HelpCircle, Edit2, Trash2, Copy, Check, Share2, ExternalLink } from 'lucide-react';
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="space-y-2">
                      {responses.map((response) => (
                        <div
                          key={response.id}
                          className="p-4 bg-surface/30 rounded-lg border border-border"
                        >
                          <div className="flex items-start justify-between mb-2">
                            <div>
                              <p className="text-white font-medium">{response.guest_name}</p>
                          ...
                          {response.response === 'yes' && (
                            <p className="text-sm text-gray-400">
                              Guests: {response.guests_count}
                            </p>
                          )}
                          {response.dietary_restrictions && (
                            <p className="text-sm text-gray-400 mt-1">
                              Dietary: {response.dietary_restrictions}
                            </p>
                          )}
                          {response.notes && (
                            <p className="text-sm text-gray-400 mt-1">Note: {response.notes}</p>
                          )}
                        </div>
                      ))}
                    </div>
