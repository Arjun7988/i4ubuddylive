...
import { Shield, Users, Package, CheckCircle, XCircle, Eye, Edit2, Trash2 } from 'lucide-react';
...
export function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'users' | 'classifieds'>('overview');
  ...
  const loadAdminData = async () => {
    try {
      const [usersResult, classifiedsResult] = await Promise.all([
        supabase
          .from('profiles')
