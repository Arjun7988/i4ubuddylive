...
import { InviteTextEditor, type InviteTextValues } from '../components/InviteTextEditor';
...
export function RSVPCreatePage() {
  ...
  eventValues={{
    title: formData.title,
    subtitle: formData.description,        // or separate field if you add one
    event_datetime:
      formData.event_date && formData.event_time
        ? `${formData.event_date} ${formData.event_time}`
        : "",
    location: location.address,
    extra_message: "",                     // add if you use it
  }}
