...
export function ClassifiedsListPage() {
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <main className="relative flex-1 pt-[calc(128px+50px)] max-w-7xl mx-auto px-4 py-8 w-full">
        <ClassifiedsBanner />
          ...
          <div className="lg:col-span-3">
            {loading ? (
              ...
              <>
                ...
                <>
                  {Array.from({ length: Math.ceil(classifieds.length / 4) }).map((_, chunkIndex) => {
                    const start = chunkIndex * 4;
                    const chunk = classifieds.slice(start, start + 4);

                    return (
                      <div key={`chunk-${chunkIndex}`}>
                        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 mb-6">
                          {chunk.map((classified) => (
                            <ClassifiedCard key={classified.id} classified={classified} />
                          ))}
                        </div>
                        {start + 4 < classifieds.length && (
                          <div className="mb-6">
